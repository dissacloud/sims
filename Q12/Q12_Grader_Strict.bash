...
#  A) PackageName: libcrypto3 + PackageVersion: <ver>
#  B) purl for libcrypto3@<ver>
#  C) cpe contains <ver> (less ideal but present in your output)
if [[ -f "$SPDX" && -n "$TARGET_VER" ]]; then
  ...
  elif grep -qE "pkg:apk/alpine/libcrypto3@${TARGET_VER//./\\.}" "$SPDX" >/dev/null 2>&1; then
    add_pass "SPDX contains libcrypto3 purl with version $TARGET_VER"
  elif grep -qE "cpe:2\\.3:a:libcrypto3:libcrypto3:${TARGET_VER//./\\.}" "$SPDX" >/dev/null 2>&1; then
