#!/usr/bin/env bash
set -euo pipefail

NS="${NS:-mtls}"
DECOY_NS="${DECOY_NS:-mtls-decoy}"
MISLEAD_NS="${MISLEAD_NS:-mtls1}"

ISTIO_VERSION="${ISTIO_VERSION:-1.22.3}"
NETSHOOT_IMAGE="${NETSHOOT_IMAGE:-nicolaka/netshoot:v0.13}"

fail(){ echo "[ERROR] $*" >&2; exit 2; }

ensure_istio() {
  if kubectl get ns istio-system >/dev/null 2>&1; then
    echo "[OK] istio-system already exists"
    return 0
  fi

  echo "[WARN] istio-system namespace not found. Installing Istio..."

  if ! command -v curl >/dev/null 2>&1; then
    sudo apt-get update -y >/dev/null
    sudo apt-get install -y curl >/dev/null
  fi

  echo "[INFO] Downloading Istio ${ISTIO_VERSION}..."
  curl -fsSL https://istio.io/downloadIstio | ISTIO_VERSION="${ISTIO_VERSION}" sh - >/dev/null

  local ISTIO_DIR="istio-${ISTIO_VERSION}"
  [ -d "${ISTIO_DIR}" ] || fail "Istio directory ${ISTIO_DIR} not found after download."

  export PATH="$PWD/${ISTIO_DIR}/bin:$PATH"
  command -v istioctl >/dev/null 2>&1 || fail "istioctl not found after download (PATH issue)."

  echo "[INFO] Installing Istio control plane (demo profile)..."
  istioctl install -y --set profile=demo >/dev/null

  echo "[INFO] Waiting for istiod..."
  kubectl -n istio-system rollout status deploy/istiod --timeout=240s >/dev/null

  echo "[OK] Istio installed"
}

require_istiod_endpoints() {
  echo "[INFO] Ensuring istiod is ready and has endpoints..."
  kubectl -n istio-system rollout status deploy/istiod --timeout=240s >/dev/null || true
  local eps
  eps="$(kubectl -n istio-system get endpoints istiod -o jsonpath='{.subsets[*].addresses[*].ip}' 2>/dev/null || true)"
  if [ -z "${eps}" ]; then
    echo "[WARN] istiod has no endpoints. Restarting istiod..."
    kubectl -n istio-system rollout restart deploy/istiod >/dev/null
    kubectl -n istio-system rollout status deploy/istiod --timeout=240s >/dev/null
  fi
  eps="$(kubectl -n istio-system get endpoints istiod -o jsonpath='{.subsets[*].addresses[*].ip}' 2>/dev/null || true)"
  [ -n "${eps}" ] || fail "istiod has no endpoints; webhook is fail-closed and will block pod creation."
  echo "[OK] istiod endpoints: ${eps}"
}

force_classic_injection_only() {
  echo "[INFO] Forcing classic injection only (istio-injection=enabled) ..."

  kubectl get mutatingwebhookconfiguration istio-sidecar-injector >/dev/null 2>&1 \
    || fail "istio-sidecar-injector webhook missing (Istio install incomplete)."

  kubectl get mutatingwebhookconfiguration istio-revision-tag-default >/dev/null 2>&1 \
    || fail "istio-revision-tag-default webhook missing (Istio install incomplete)."

  # Enable classic injector: namespaceSelector matchLabels istio-injection=enabled
  # Remove objectSelector blocks (set to empty selector).
  kubectl patch mutatingwebhookconfiguration istio-sidecar-injector --type='json' -p='[
    {"op":"remove","path":"/webhooks/0/namespaceSelector"},
    {"op":"add","path":"/webhooks/0/namespaceSelector","value":{"matchLabels":{"istio-injection":"enabled"}}},
    {"op":"remove","path":"/webhooks/1/namespaceSelector"},
    {"op":"add","path":"/webhooks/1/namespaceSelector","value":{"matchLabels":{"istio-injection":"enabled"}}},
    {"op":"remove","path":"/webhooks/2/namespaceSelector"},
    {"op":"add","path":"/webhooks/2/namespaceSelector","value":{"matchLabels":{"istio-injection":"enabled"}}},
    {"op":"remove","path":"/webhooks/3/namespaceSelector"},
    {"op":"add","path":"/webhooks/3/namespaceSelector","value":{"matchLabels":{"istio-injection":"enabled"}}},

    {"op":"remove","path":"/webhooks/0/objectSelector"},
    {"op":"add","path":"/webhooks/0/objectSelector","value":{}},
    {"op":"remove","path":"/webhooks/1/objectSelector"},
    {"op":"add","path":"/webhooks/1/objectSelector","value":{}},
    {"op":"remove","path":"/webhooks/2/objectSelector"},
    {"op":"add","path":"/webhooks/2/objectSelector","value":{}},
    {"op":"remove","path":"/webhooks/3/objectSelector"},
    {"op":"add","path":"/webhooks/3/objectSelector","value":{}}
  ]' >/dev/null

  # Disable revision-tag injector: make it never match
  kubectl patch mutatingwebhookconfiguration istio-revision-tag-default --type='json' -p='[
    {"op":"remove","path":"/webhooks/0/namespaceSelector"},
    {"op":"add","path":"/webhooks/0/namespaceSelector","value":{"matchLabels":{"istio.io/deactivated":"never-match"}}},
    {"op":"remove","path":"/webhooks/1/namespaceSelector"},
    {"op":"add","path":"/webhooks/1/namespaceSelector","value":{"matchLabels":{"istio.io/deactivated":"never-match"}}},
    {"op":"remove","path":"/webhooks/2/namespaceSelector"},
    {"op":"add","path":"/webhooks/2/namespaceSelector","value":{"matchLabels":{"istio.io/deactivated":"never-match"}}},
    {"op":"remove","path":"/webhooks/3/namespaceSelector"},
    {"op":"add","path":"/webhooks/3/namespaceSelector","value":{"matchLabels":{"istio.io/deactivated":"never-match"}}}
  ]' >/dev/null

  echo "[OK] Classic injector active; revision-tag injector disabled."
  echo "Classic selector:"
  kubectl get mutatingwebhookconfiguration istio-sidecar-injector -o jsonpath='{.webhooks[0].namespaceSelector}{"\n"}'
  echo "Revision-tag selector:"
  kubectl get mutatingwebhookconfiguration istio-revision-tag-default -o jsonpath='{.webhooks[0].namespaceSelector}{"\n"}'
}

echo "== Q15 (Classic) Lab Setup =="
echo "Target namespace: ${NS}"
echo "Decoy namespace:  ${DECOY_NS}"
echo "Mislead namespace:${MISLEAD_NS}"
echo "Istio version:    ${ISTIO_VERSION}"
echo "Workload image:   ${NETSHOOT_IMAGE}"
echo

ensure_istio
require_istiod_endpoints
force_classic_injection_only

# Namespaces
kubectl get ns "${NS}" >/dev/null 2>&1 || kubectl create ns "${NS}" >/dev/null
kubectl get ns "${DECOY_NS}" >/dev/null 2>&1 || kubectl create ns "${DECOY_NS}" >/dev/null
kubectl get ns "${MISLEAD_NS}" >/dev/null 2>&1 || kubectl create ns "${MISLEAD_NS}" >/dev/null

# Target starts BROKEN: no injection
kubectl label ns "${NS}" istio-injection- --overwrite >/dev/null 2>&1 || true
kubectl label ns "${NS}" istio.io/rev- --overwrite >/dev/null 2>&1 || true

# Decoys already injected (trap): classic label only
kubectl label ns "${DECOY_NS}" istio-injection=enabled --overwrite >/dev/null
kubectl label ns "${DECOY_NS}" istio.io/rev- --overwrite >/dev/null 2>&1 || true
kubectl label ns "${MISLEAD_NS}" istio-injection=enabled --overwrite >/dev/null
kubectl label ns "${MISLEAD_NS}" istio.io/rev- --overwrite >/dev/null 2>&1 || true

# Workloads in target namespace
kubectl -n "${NS}" delete deploy,svc --all --ignore-not-found >/dev/null 2>&1 || true
kubectl -n "${NS}" delete peerauthentication default --ignore-not-found >/dev/null 2>&1 || true

cat <<EOF | kubectl -n "${NS}" apply -f -
apiVersion: v1
kind: Service
metadata:
  name: tcp-echo
spec:
  selector:
    app: tcp-echo
  ports:
  - name: tcp
    port: 9000
    targetPort: 9000
---
apiVersion: apps/v1
kind: Deployment
metadata:
  name: tcp-echo
spec:
  replicas: 1
  selector:
    matchLabels:
      app: tcp-echo
  template:
    metadata:
      labels:
        app: tcp-echo
    spec:
      containers:
      - name: server
        image: ${NETSHOOT_IMAGE}
        command: ["/bin/sh","-lc"]
        args:
          - |
            apk add --no-cache socat >/dev/null 2>&1 || true
            while true; do socat -T 1 -v TCP-LISTEN:9000,reuseaddr,fork EXEC:'/bin/cat'; done
        ports:
        - containerPort: 9000
---
apiVersion: apps/v1
kind: Deployment
metadata:
  name: tcp-client
spec:
  replicas: 1
  selector:
    matchLabels:
      app: tcp-client
  template:
    metadata:
      labels:
        app: tcp-client
    spec:
      containers:
      - name: client
        image: ${NETSHOOT_IMAGE}
        command: ["/bin/sh","-lc"]
        args:
          - |
            sleep 365d
EOF

# Baseline PERMISSIVE (candidate must set STRICT)
cat <<EOF | kubectl apply -f -
apiVersion: security.istio.io/v1beta1
kind: PeerAuthentication
metadata:
  name: default
  namespace: ${NS}
spec:
  mtls:
    mode: PERMISSIVE
EOF

# Decoy STRICT in istio-system
cat <<'EOF' | kubectl apply -f -
apiVersion: security.istio.io/v1beta1
kind: PeerAuthentication
metadata:
  name: strict-decoy
  namespace: istio-system
spec:
  mtls:
    mode: STRICT
EOF

kubectl -n "${NS}" rollout status deploy/tcp-echo --timeout=180s >/dev/null
kubectl -n "${NS}" rollout status deploy/tcp-client --timeout=180s >/dev/null

echo
echo "== Lab state summary (Classic-only) =="
kubectl get ns "${NS}" --show-labels
kubectl -n "${NS}" get pods -o wide
kubectl -n "${NS}" get peerauthentication default -o jsonpath='{.spec.mtls.mode}{"\n"}'
echo "Run: ./question.sh"
